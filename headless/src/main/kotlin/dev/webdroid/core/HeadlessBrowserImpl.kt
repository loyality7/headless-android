...
internal class HeadlessBrowserImpl(
    ...
    init {
        dev.webdroid.platform.MemoryPressureMonitor.register(appContext)
    }
    ...
    override suspend fun newPage(viewport: Viewport?): Page = sessionLock.withLock {
        ...
        if (sessions.size >= config.maxSessions) {
            throw dev.webdroid.browserError(
                dev.webdroid.ErrorCode.MEMORY_LIMIT,
    ...
    private fun checkNotClosed() {
        if (isClosed.get()) {
            throw dev.webdroid.browserError(
                dev.webdroid.ErrorCode.DETACHED,
                "browser is closed"
            )
        }
    }
