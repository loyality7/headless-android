...
import dev.webdroid.protocol.CdpChannel
import dev.webdroid.protocol.ProtocolCommandEngine
import dev.webdroid.protocol.ProtocolTargetDiscovery
import dev.webdroid.protocol.WebSocketClient
...
class LiveRealSitesDeviceTest {
    ...
    fun testLiveRendererRecoveryOnDevice() = runBlocking {
        ...
        val navigator = PlatformNavigator(session, config)
        val reader = PlatformReader(session, dev.webdroid.platform.PlatformScriptEngine(session, config), config)
